...
PROGRAM W3OUNF
  ...
  USE W3SERVMD, ONLY : ITRACE, NEXTLN, EXTCDE, EXTOPN, EXTIOF, STR_TO_UPPER
#ifdef W3_S
  ...
  IF (FLGNML) THEN
    ...
    CALL W3FLGRDFLAG ( NDSO, SCREEN, NDSE, FLDOUT, FLG1D,       &
         FLG2D, 1, 1, IERR )
    IF (IERR.NE.0) CALL EXTOPN(NDSE,IERR,'W3OUNF','INPUT',10)
  ...
  !
  IF (.NOT. FLGNML) THEN
    OPEN (NDSI,FILE=TRIM(FNMPRE)//'ww3_ounf.inp',STATUS='OLD',IOSTAT=IERR)
    IF (IERR.NE.0) CALL EXTOPN(NDSE,IERR,'W3OUNF','INPUT',10)
    REWIND (NDSI)
    ...
    !        CALL NEXTLN ( COMSTR , NDSI , NDSE )
    !        READ (NDSI,*,IOSTAT=IERR) FLGFC
    !        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3OUNF','INPUT',11)
    !        IF( FLGFC ) THEN
    !          READ(NDSI,*,IOSTAT=IERR) TREF
    !          IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3OUNF','INPUT',11)
    !        END IF
    !
    ...
    IF(SMCGRD) THEN
      ...
      READ (NDSI,*,IOSTAT=IERR) SMCOTYPE
      IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3OUNF','INPUT',11)
      IF(SMCOTYPE .EQ. 1) THEN  ! Flat sea point output
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) SXO, SYO, EXO, EYO
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3OUNF','INPUT',11)
      ELSE IF(SMCOTYPE .EQ. 2) THEN  ! Regular grid output
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
        READ (NDSI,*,IOSTAT=IERR) SXO, SYO, EXO, EYO, CELFAC
        IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3OUNF','INPUT',11)
      ENDIF
      SMCNOVAL = NOVAL
#endif
          ...
          DO WHILE(LOOP)
            ...
            IF (.NOT.FEXIST) THEN
              ...
              CALL W3CRNC(FNAMENC,NCID,DIMID,DIMLN,VARID, &
                   EXTRADIM,NCTYPE,NCDEFLATE, MAPSTAOUT)
            ...
            END IF
            !
          END DO !  Loop over IPART for partition variables
...
9020 FORMAT (' TEST W3EXNC : OUTPUT FIELD : ',A)
  ...
  SUBROUTINE W3CRNC (NCFILE, NCID, DIMID, DIMLN, VARID,  &
    ...
    INTEGER, INTENT(IN)               :: NCTYPE, NCDEFLATE
    ...
    IF(FL_DEFAULT_GBL_META) THEN
      ...
      IF (ZZ0MAX.NE.0.)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter Z0MAX',ZZ0MAX)
      IF (SSINTHP.NE.2.)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SINTHP',SSINTHP)
      IF (ZZALP.NE.2.)      IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter ZALP',ZZALP)
      IF (TTAUWSHELTER.NE.0.3) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter TAUWSHELTER',TTAUWSHELTER)
      IF (ZZ0RAT.NE.0.04)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter Z0RAT',ZZ0RAT)
      IF (SSINBR.NE.0.0)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SINBR',SSINBR)
      IF (SINTAILPAR(1).NE.1.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SINTABLE',NINT(SINTAILPAR(1)))
      IF (SINTAILPAR(2).NE.0.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SINTAIL1',SINTAILPAR(1))
      IF (SINTAILPAR(3).NE.0.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SINTAIL2',SINTAILPAR(1))
      IF (SINTAILPAR(4).NE.1.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter TAUWBUG',NINT(SINTAILPAR(4)))
      IF (SINTAILPAR(5).NE.1.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter VISCSTRESS',SINTAILPAR(5))
      IF (CAPCHNK(1).NE.0.0) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter CAPCHA',NINT(CAPCHNK(1)))
      IF (CAPCHNK(2).NE.0.0001) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter CHAMIN',CAPCHNK(2))
      IF (CAPCHNK(3).NE.0.0095) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter CHA0',CAPCHNK(3))
      IF (CAPCHNK(4).NE.30.) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter UCAP' ,CAPCHNK(4))
      IF (CAPCHNK(5).NE.10.) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SIGMAUCAP',CAPCHNK(5))
      IF (SSWELLF(1).NE.0.66)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF',SSWELLF(1))
      IF (SSWELLF(2).NE.-0.018)   IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF2',SSWELLF(2))
      IF (SSWELLF(3).NE.0.022)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF3',SSWELLF(3))
      IF (SSWELLF(4).NE.1.5E5)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF4',SSWELLF(4))
      IF (SSWELLF(5).NE.1.2)      IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF5',SSWELLF(5))
      IF (SSWELLF(6).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF6',SSWELLF(6))
      IF (SSWELLF(7).NE.360000.)  IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SIN4 namelist parameter SWELLF7',SSWELLF(7))

      IF(SSDSC(1).NE.1.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSBCHOICE', NINT(SSDSC(1)))
      IF(SSDSC(2).NE.-2.2E-5)   IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSC2', SSDSC(2))
      IF(SSDSC(3).NE.-0.40344)  IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSCUM', SSDSC(3))
      IF(SSDSC(4).NE.1.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSBCHOICE', SSDSC(4))
      IF(SSDSC(5).NE.0.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSBCHOICE', SSDSC(5))
      IF(SSDSC(6).NE.0.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSBCHOICE', SSDSC(6))
      IF(SSDSC(7).NE.0.3)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter WHITECAPWIDTH', SSDSC(7))
      IF(SSDSC(8).NE.0.)        IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSSTRAIN', SSDSC(8))
      IF(SSDSC(9).NE.3.5000e-05)IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSL', SSDSC(9))
      !IF(SSDSC(10).NE.15*NTH/360)   IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSSTRAINA*NTH/360', SSDSC(10))
      IF(SSDSC(11).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSSTRAIN2', SSDSC(11))
      IF(SSDSC(12).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter CUMSIGP', SSDSC(12))
      IF(SSDSC(13).NE.0.9)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSMWD', SSDSC(13))
      IF(SSDSC(14).NE.0.5)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SPMSS', SSDSC(14))
      IF(SSDSC(15).NE.1.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSMWPOW', SSDSC(15))
      IF(SSDSC(16).NE.3.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDKOF', SSDSC(16))
      IF(SSDSC(17).NE.0.56)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter WHITECAPDUR', SSDSC(17))
      IF(SSDSC(18).NE.400)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSFACMTF', SSDSC(18))
      IF(SSDSC(19).NE.1.5)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSNMTF', SSDSC(19))
      IF(SSDSC(20).NE.2.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSCUMP', SSDSC(20))
      IF(SSDSC(21).NE.0.0)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSNUW', SSDSC(21))
      IF(SSDSBR.NE.0.90E-3)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SDS4 namelist parameter SDSBR', SSDSBR)
#endif
#ifdef W3_NL1
      IF (SNLC1.NE.2.50E7)      IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter NLPROP',SNLC1)
      IF (LAM.NE.0.)            IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter LAMBDA',LAM)
      IF (KDCON.NE.0.75)        IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter KDCONV',KDCON)
      IF (KDMN.NE.0.50)         IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter KDMIN',KDMN)
      IF (SNLS1.NE.5.5)         IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter SNLCS1',SNLS1)
      IF (SNLS2.NE.0.833)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter SNLCS2',SNLS2)
      IF (SNLS3.NE.-1.25)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter SNLCS3',SNLS3)
      IF (IQTPE.NE.1)           IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter IQTYPE',IQTPE)
      IF (NLTAIL.NE.-5)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter TAILNL',NLTAIL)
      IF (GQNF1.NE.14)          IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMNF1',GQNF1)
      IF (GQNT1.NE.8)           IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMNT1',GQNT1)
      IF (GQNQ_OM2.NE.8)        IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMNQ_OM2',GQNQ_OM2)
      IF (GQTHRSAT.NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMTHRSAT',GQTHRSAT)
      IF (GQFRESAT.NE.1.27)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMFRESAT',GQFRESAT)
      IF (GQTHRCOU.NE.0.015)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMTHRCOU',GQTHRCOU)
      IF (GQAMP(1).NE.1.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQAMP1',GQAMP(1))
      IF (GQAMP(2).NE.0.0022)   IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQAMP2',GQAMP(2))
      IF (GQAMP(3).NE.1.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQAMP3',GQAMP(3))
      IF (GQAMP(4).NE.1.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQAMP4',GQAMP(4))
      IF (GQMDIA.NE.0)          IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMDIA',GQMDIA)
      IF (GQMDIA_WND_THR.NE.15.) IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMDIA_WND_THR',GQMDIA_WND_THR)
      IF (GQMDIA_HS_THR.NE.6)   IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMDIA_HS_THR',GQMDIA_HS_THR)
      IF (GQMDIAFIN.NE.0.24)    IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMDIAFIN',GQMDIAFIN)
      IF (GQMDIAFDS.NE.0.4)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'SNL1 namelist parameter GQMDIAFDS',GQMDIAFDS)
#endif
#ifdef W3_REF1
      IF (REFPARS(1).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFCOAST',REFPARS(1))
      IF (REFPARS(2).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFSUBGRID',REFPARS(2))
      IF (REFPARS(3).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFUNSTSOURCE',REFPARS(3))
      IF (REFPARS(4).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFICEBERG',REFPARS(4))
      IF (REFPARS(6).NE.0.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFFREQ',REFPARS(6))
      IF (REFPARS(7).NE.0.22)     IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFSLOPE',REFPARS(7))
      IF (REFPARS(8).NE.4.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFCOSP_STRAIGHT',REFPARS(8))
      IF (REFPARS(9).NE.1.)       IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFRMAX',REFPARS(9))
      IF (REFPARS(10).NE.2.)      IRET=NF90_PUT_ATT(NCID,NF90_GLOBAL,'REF1 namelist parameter REFFREQPOW',REFPARS(10))
#endif
